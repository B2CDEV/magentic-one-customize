...
from autogen_magentic_one.agents.coder import Coder, Reservation, Executor, Basic
from autogen_magentic_one.agents.file_surfer import FileSurfer, TunedFileSurfer
...
async def main(logs_dir: str, hil_mode: bool, save_screenshots: bool) -> None:
    ...
    async with DockerCommandLineCodeExecutor(work_dir=logs_dir) as code_executor:
        ...
        await Reservation.register(runtime, "reservation", lambda: Reservation(model_client=client))
        reservation = AgentProxy(AgentId("reservation", "default"), runtime)

        # await Basic.register(runtime, "basic", lambda: Basic(model_client=client))
        # basic = AgentProxy(AgentId("basic", "default"), runtime)

        await Executor.register(
            runtime,
            "Executor",
            lambda: Executor("A agent for executing code", executor=code_executor, confirm_execution="ACCEPT_ALL"),
        )
        ...
        # await TunedFileSurfer.register(runtime, "file_surfer", lambda: TunedFileSurfer(model_client=client))
        # file_surfer = AgentProxy(AgentId("file_surfer", "default"), runtime)
        ...
        agent_list = [executor, file_surfer, reservation]
        if hil_mode:
