...
class FileSurfer(BaseWorker):
    ...
    DEFAULT_SYSTEM_MESSAGES = [
        ...
        If the previous conversation was about reservations, read request_reservation.py and pass the code block verbatim.
...
class TunedFileSurfer(FileSurfer) :
    DEFAULT_DESCRIPTION = "An agent that finds the code file that makes the reservation when all the reservation information is collected."
    DEFAULT_SYSTEM_MESSAGES = [
        SystemMessage(
            content="""
            You are a helpful AI Assistant.
            When given a user query, use available functions to help the user with their request.
            Read the 'reservation_system.py' file.
            """
        ),
    ]